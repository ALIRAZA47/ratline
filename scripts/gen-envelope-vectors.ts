...
/**
 * Identity vectors (RL-M2-005). Same arrangement as the envelope's, for the same reason:
 * two independent implementations of a byte layout can each be self-consistent and still
 * disagree, and the failure arrives on a host as "the agent cannot authenticate".
 *
 * The host key is generated here and its private half committed, because these vectors
 * are worthless without the ability to reproduce the signature. It is a test key.
 */
const { privateKey: hostPrivate, publicKey: hostPublic } = generateKeyPairSync("ed25519");
const hostPrivatePem = hostPrivate.export({ type: "pkcs8", format: "pem" }).toString();

const identityVectors = [
  { name: "ordinary", nonce: "a".repeat(64) },
  { name: "all-zero-nonce", nonce: "0".repeat(64) },
  // The ambiguity length prefixes remove: without them, a long host id and a short nonce
  // could flatten to the same bytes as the reverse.
  { name: "boundary-lengths", nonce: "f".repeat(64) },
].map((testCase) => ({
  name: testCase.name,
  host_id: HOST,
  nonce: testCase.nonce,
  signature: signChallenge(hostPrivatePem, HOST, testCase.nonce),
  signed_hex: Buffer.from(challengeBytes(HOST, testCase.nonce)).toString("hex"),
}));

mkdirSync(TARGET_DIR, { recursive: true });
writeFileSync(
  join(TARGET_DIR, "identity_vectors.json"),
  `${JSON.stringify(
    {
      note:
        "Generated by scripts/gen-envelope-vectors. Verified by " +
        "agent/internal/protocol/identity_security_test.go. If either implementation " +
        "changes the bytes it signs, its verification of these signatures fails.",
      domain: IDENTITY_DOMAIN,
      host_id: HOST,
      test_only_private_key_pem: hostPrivatePem,
      public_key_raw_hex: hostPublic
...
console.log(
  `wrote agent/internal/protocol/testdata/identity_vectors.json — ${String(identityVectors.length)} vectors`,
);
