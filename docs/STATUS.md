# Ratline — status

<!-- GENERATED BY `scripts/tasks render`. Do not hand-edit; your changes will be overwritten. -->
Generated 2026-08-03.

## Milestones

| Milestone | Progress | Done | Total | Open high-risk |
| --- | --- | ---: | ---: | ---: |
| **M0** Plan | `########################` 100% | 26 | 26 | 0 |
| **M1** Control plane skeleton | `######################--` 93% | 43 | 46 | 2 |
| **M2** Agent and first host | `------------------------` 0% | 0 | 29 | 20 |
| **M3** Static sites end to end | `------------------------` 0% | 0 | 27 | 18 |
| **M4** Node and Bun apps | `------------------------` 0% | 0 | 15 | 10 |
| **M5** RBAC completion | `------------------------` 0% | 0 | 7 | 6 |
| **M6** SSH access | `------------------------` 0% | 0 | 7 | 7 |
| **M7** Managed databases | `------------------------` 0% | 0 | 16 | 15 |

## Current milestone — M1: Control plane skeleton

Auth, org/team/project model, the can() function with 100% branch coverage, hash-chained audit log, UI shell with the design system applied, tracking CLI wired into CI.

### Exit criteria

- [ ] Authorization matrix test harness exists and runs, even though most endpoints do not exist yet
- [ ] `tasks validate` gates the build

0 of 2 met.

## In progress

_None._

## Blocked

| Task | Age | Owner | Risk | Acceptance | Title |
| --- | ---: | --- | --- | ---: | --- |
| `RL-M2-010` | 1d | agent | **high** | 0/4 | Implement agent installation over SSH as the only raw SSH path |
| `RL-M2-011` | 1d | agent | medium | 0/3 | Make bootstrap idempotent and resumable |
| `RL-M2-012` | 1d | agent | low | 0/3 | Collect host inventory |
| `RL-M2-013` | 1d | agent | low | 0/3 | Implement health reporting and heartbeat |
| `RL-M2-015` | 1d | agent | **high** | 0/3 | Provision base packages and unattended security upgrades |
| `RL-M2-016` | 1d | agent | medium | 0/3 | Install and baseline the web server |
| `RL-M2-019` | 1d | agent | medium | 0/3 | Install the runtime version manager and pin runtimes per site |
| `RL-M2-020` | 1d | agent | **high** | 0/4 | Create per-site Linux users and the site directory layout |
| `RL-M2-021` | 1d | agent | **high** | 0/3 | Manage the host firewall |
| `RL-M2-022` | 1d | agent | medium | 0/3 | Apply an intrusion response policy for repeated authentication failures |
| `RL-M2-023` | 1d | agent | **high** | 0/4 | Implement agent self-update with rollback |
| `RL-M2-024` | 1d | human | **high** | 0/3 | Build the integration test harness against real hosts |
| `RL-M2-025` | 1d | agent | medium | 0/3 | Prove provisioning is idempotent end to end |
| `RL-M2-026` | 1d | agent | **high** | 0/3 | Chaos test the provisioning path against connection loss |
| `RL-M2-029` | 1d | agent | **high** | 0/3 | Verify zero root SSH remains after bootstrap |

## In review

| Task | Age | Owner | Risk | Acceptance | Title |
| --- | ---: | --- | --- | ---: | --- |
| `RL-M1-002` | 2d | agent | low | 1/3 | Build the CI pipeline and wire the tracker into it |
| `RL-M1-020` | 1d | agent | **high** | 2/3 | Rate limit every authentication path |

## Open high-risk tasks without security-test coverage

These cannot be marked `done` until they name a security-suite artifact (brief §2.2).

- `RL-M2-021` (M2, blocked) — Manage the host firewall
- `RL-M2-024` (M2, blocked) — Build the integration test harness against real hosts
- `RL-M2-026` (M2, blocked) — Chaos test the provisioning path against connection loss
- `RL-M3-010` (M3, todo) — Implement one-click rollback
- `RL-M3-013` (M3, todo) — Issue certificates over HTTP validation
- `RL-M4-003` (M4, todo) — Implement zero-downtime reload
- `RL-M4-004` (M4, todo) — Implement health checks and automatic abort
- `RL-M4-015` (M4, todo) — Implement control plane self-update with rollback
- `RL-M6-003` (M6, todo) — Enforce principals host-side and make revocation immediate
- `RL-M7-001` (M7, todo) — Re-examine the failure story for workloads that cannot be rebuilt
- `RL-M7-006` (M7, todo) — Restore a backup into a scratch instance and verify it byte-for-byte in CI
- `RL-M7-013` (M7, todo) — Connect a database to a site and prove the credential path end to end
- `RL-M7-015` (M7, todo) — Implement MongoDB support
- `RL-M7-016` (M7, todo) — Decide how major-version upgrades are handled, or that they are not
- `RL-M1-046` (M1, todo) — Chase the intermittent failure in the no-default-secrets suite

## Test health

| Suite | Passing | Rate |
| --- | ---: | ---: |
| Unit | 390 / 390 | 100% |
| Integration | not measured | — |
| Authorization matrix | 191 / 191 | 100% |
| `src/authz/**` line coverage | 100% | must be 100% |
| `can()` branch coverage | 100% | must be 100% |

Measured 2026-08-02T16:21:59.739Z.

### Authorization matrix (role × endpoint × subject)

| | |
| --- | ---: |
| Cells passing | 588 / 588 |
| Endpoints × roles | 28 × 7 |
| Verified end to end (real request) | 56 |
| Verified at the decision layer (`can()`) | 448 |
| Unguarded by declaration, nothing to decide | 84 |

> The remainder is verified one layer down, against `can()` with real grants and
> row-level security. Two things are not expressible as a request: a route the
> server does not bind, and a cross-tenant subject on a route with no identifier
> in its path — the tenant comes from the session, so a URL cannot name another.

## Security findings

| Severity | Open |
| --- | ---: |
| Critical | 0 |
| High | 0 |
| Medium | 0 |

Gate check (critical + high must be zero): PASS

## Build

CI: _no status recorded yet._

## Totals

| Status | Count |
| --- | ---: |
| todo | 87 |
| blocked | 15 |
| review | 2 |
| done | 69 |
| **all** | **173** |
