...
| R-23 | ~~Nothing calls the CSRF guard.~~ **Closed by RL-M1-042.** `createServer` runs `guardCsrf` on every unsafe method, and the authorization matrix drives real POSTs through it — a request without a token is refused, which the matrix caught the first time by failing three cells. Originally: There is no HTTP layer, so no cookie carries these attributes and no request is refused in production | Identical in shape to R-15's note on the rate limiter, and owed by the same work: `src/api/server.ts` does not exist and RL-M1-021 must not create it. The module and its policy are exercised end to end by `test/security/csrf.test.ts`, and the wiring is enumerated at the bottom of `src/api/csrf.ts`. Until it lands this is a mechanism, not a protection |
...
| R-26 | An administrative reset of a second factor removes the bound that makes R-17 survivable. `member.reset_password` alone cannot take an account because the second factor still stands; whoever holds BOTH can take one outright | Closed as the capability (RL-M1-038) — an operator whose colleague has lost their authenticator and their recovery codes now has a route that is not editing the database. Bounded the same three ways as R-17: `member.reset_two_factor` is catalogued and held only by Owner and Admin, the reset is audited against the operator before the account can be used, and the member is signed out so they notice. The two actions are deliberately held by the same roles and a test asserts they move together — splitting them would suggest the pair is safer than either, and it is the opposite. The residual is the pair itself, which no arrangement of either action can close: it is the definition of an administrator |
...
| R-28 | ~~Two-factor verification is not rate limited in effect.~~ **Closed.** `POST /auth/two-factor` exists and spends `AUTH_RATE_LIMITS["two-factor"]` before it verifies anything, proven by a test that drives the endpoint past its limit and reads the counter. The bucket keys on the challenge token, which needs no lookup — and a fresh challenge cannot buy fresh attempts, because minting one requires a successful sign-in and sign-in is limited on the account and the address, so the two budgets multiply. Originally: a six-digit code is a million possibilities and an unlimited verifier finds one inside a day |
| R-29 | **Nine repository reads resolved no permission at all** — `listAudit`, `verifyAuditChain`, the two verification reads, and the organization/member/project reads. `audit_log.read` is Owner and Admin only, so every authenticated member of a tenant could read the entire audit log | Closed (RL-M1-043). Row-level security still confined all of them to one tenant, so it was a within-tenant privilege gap rather than a cross-tenant one, and nothing called them over HTTP because there was no server AT THE TIME — the only reason it was never live. RL-M1-042 built one and RL-M1-055 runs it, so that mitigating fact has expired: this class is live now, which is why the gating rule rather than the nine fixes is the durable artifact. It went unnoticed because C3's three layers all HELD: an unreachable handle, a branded context and forced RLS none of which has an opinion about WHICH member of a tenant is asking. That is the question `can()` answers, and these functions never asked it. The durable fix is `test/security/repository_gating.test.ts`: every exported repository function resolves a permission or carries a written argument for why not |
| R-30 | **Only 56 of 630 matrix cells are verified end to end.** The server binds 12 of 30 declared routes, and a cross-tenant subject cannot be expressed as a URL on a route with no identifier in its path | Not a defect, and reported as a number rather than a caveat (STATUS.md). The remainder is verified against `can()` with real grants and forced row-level security, which proves the permission model and not that a route consults it. Every route gains transport coverage as its repository function is written; a route the server refuses to stub is honestly absent rather than falsely reachable. **Corrected 2026-08-04:** this entry read "56 of 567" and "8 of 27", the same two wrong numbers the M1 gate report corrected in itself and did not propagate here. 27 was invented; "eight" is the count of transport-expressible cells PER ROLE (8 x 7 = 56). `transportCoverage()` answers `{bound: 12, declared: 30}` and always did. Found by a documentation pass reading every file at once |
