...
| R-22 | The TOTP shared secret is a symmetric credential the control plane can impersonate its holder with. Whoever holds the database AND the key-encryption key file can mint valid codes for anybody | Inherent to TOTP; the scheme that removes it is WebAuthn, where the verifier stores only a public key. Mitigated by envelope encryption (ADR 0006 applied in migration 12), so the database alone is not enough, and by the key file being excluded from database backups. Scoped out of RL-M1-019 rather than overlooked |
| R-23 | There is no administrative reset of a second factor. A member who loses both their authenticator and their recovery codes has no route back except the operator editing the database | Deliberate. The reset action turns off anybody's second factor, so it is the single most dangerous capability in the feature and `src/authz/catalogue.ts` has no action to check an administrator against. Shipping the capability with the check "to follow" is exactly what brief §9 rejects. `src/repo/two_factor.ts` refuses to write anyone else's enrolment at all — the answer `updatePasswordHash` already gives to the same question (R-17) |
| R-24 | Beginning a re-enrolment makes the existing enrolment pending, so a person who abandons it has no second factor until they finish. Somebody holding a stolen session can strip a victim's factor this way | Bounded: the attacker already holds a session that passed the factor, so this costs the victim a re-enrolment rather than costing them access, and a challenge (password only) cannot do it at all — that path is refused outright. Closing it means staging the new secret beside the old one, which is two more columns; named in migration 12 so the omission reads as a decision |
| R-25 | Two-factor verification is not yet rate limited in effect. `AUTH_RATE_LIMITS["two-factor"]` publishes a budget and nothing spends it, because there is no HTTP layer to spend it | The same shape as R-15 and for the same reason: the limiter must run before the work, so the ordering is the route handler's. A six-digit code is a million possibilities and an unlimited verifier finds one inside a day, which makes this the sharpest open item in the feature. Listed at the foot of `src/auth/two_factor.ts` as owed by whoever builds `src/api/` |
