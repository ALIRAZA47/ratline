...
| R-15 | The sign-in path derives a 128 MiB scrypt hash per attempt, including for accounts that do not exist, so an unauthenticated burst is a memory-exhaustion lever on a control plane the brief says must run on a laptop | Deriving for nonexistent accounts is deliberate — it stops timing revealing whether an account exists (ADR 0014). The mitigation is rate limiting on every auth path, which is RL-M1-020 and lists this task as its dependency |
